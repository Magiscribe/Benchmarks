...
from providers import PROVIDER_REGISTRY, VisionProvider
...
class ModelRunner:
    ...
    def __init__(
        ...
        max_retries: int = 3,
        retry_delay: int = 2,
        system_messages_dir: Optional[str] = None
    ):
        """
        ...
        Args:
            ...
            system_messages_dir: Path to directory containing system message files
        """
    ...
    def _create_system_prompt(self, system_messages_dir: str = None) -> str:
        """
        ...
        """
        if system_messages_dir and os.path.exists(os.path.join(system_messages_dir, "model.txt")):
            # Read from system message file
            with open(os.path.join(system_messages_dir, "model.txt"), 'r', encoding='utf-8') as f:
                return f.read().strip()
        else:
            ...
            return (
                ...
                "The font sizes are standardized and there are only capitalized and lowercase English letters in the image. "
                "\n\n"
    ...
    def run_model_on_image(self, image_path: str) -> List[Dict[str, Any]]:
        ...
        # Create the prompt
        system_prompt = self._create_system_prompt(self.system_messages_dir)
...
def parse_args():
    ...
    parser.add_argument("--model", type=str, choices=available_models.AVAILABLE_MODELS,
...
if __name__ == "__main__":
    ...
    model_id = available_models.MODELS[args.model]
