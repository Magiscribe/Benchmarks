"""
Harness for the MySQL -> Postgres migration task.

Commands:
    start    - spin up MySQL with the seed, stash eval/ outside the task tree,
               prepare workspace, start clock
    score    - stop clock, restore eval/, run eval.py, print score +
               write last_result.json
    run      - start + invoke a named runner (blocks until CLI exits) +
               score + append leaderboard row
    cleanup  - tear down harness-started containers, remove workspace,
               restore eval/ if it's still staged. --sweep also kills any
               container exposing the target Postgres port.

The agent is invoked separately by the operator (or by `run`) with
CWD = ./workspace. It signals completion by creating ./workspace/.done
(optionally JSON with {"agent": "...", "model": "...", "notes": "..."}).
"""
import argparse
import csv
import json
...
MYSQL_CONTAINER = "benchmark-mysql-legacy"
MYSQL_PORT = 3306
POSTGRES_PORT = 5433
...
def wait_for_mysql_healthy(timeout=180):
...
def restore_eval(staged_path_str):
    ...
    # Clean up the now-empty staging parent
    try:
        staged.parent.rmdir()
    except OSError:
        pass
...
def cmd_start(args):
    ...
    log("Starting MySQL container with seed...")
    r = compose("up", "-d")
    if r.returncode != 0:
    ...
    if not wait_for_mysql_healthy(timeout=180):
        log("MySQL did not become healthy in time. Dumping logs:")
        compose("logs", "mysql")
        sys.exit(1)
...
def _score_core():
    ...
    # State no longer reflects reality — eval is back on disk
...
def cmd_cleanup(args):
    ...
    log("Stopping harness MySQL container (if running)...")
    compose("down", "-v", capture=True)

    if args.sweep:
        log(f"Sweeping any containers on port {POSTGRES_PORT} (--sweep)...")
        sweep_port(POSTGRES_PORT)
    else:
        leftover = find_containers_on_port(POSTGRES_PORT)
        if leftover:
            log(f"Containers still bound to port {POSTGRES_PORT}: "
...
def main():
    parser = argparse.ArgumentParser(
        description="MySQL -> Postgres migration task harness"
    )
    sub = parser.add_subparsers(dest="cmd", required=True)

    p_start = sub.add_parser("start", help="Start MySQL and prepare workspace")
    p_start.add_argument("--agent", help="Agent identifier (e.g. claude-code)")
    p_start.add_argument("--model", help="Model identifier (e.g. opus-4.7)")

    sub.add_parser("score", help="Run eval and print score")

    p_run = sub.add_parser("run", help="start + invoke a runner + score")
    p_run.add_argument("--agent", required=True,
                       help="Runner name; must match runners/<agent>.sh")
    ...
    p_cleanup = sub.add_parser("cleanup",
                               help="Tear down harness container and workspace")
    p_cleanup.add_argument("--sweep", action="store_true",
                           help=f"Also remove containers on port {POSTGRES_PORT}")
