...
```bash
python utils/synthesize_model_results.py
```
...
- `test_config.py` - Test-specific configuration settings
- `utils/` - Utility modules
  - `dataset_creator.py` - Generates benchmark dataset
  - `image_generator.py` - Creates eye chart test images
  - `model_evaluator.py` - Evaluates model responses
  - `synthesize_model_results.py` - Creates consolidated CSV of results
- `system_messages/` - LLM prompts for evaluation
- `responses/` - Model response files
- `assets/` - Generated test images with flat naming structure (Font_timestamp.png)
